export const reactionFrag = `uniform float timestep;
uniform float k;
...
void main() {
	...
	float reaction = max(getTex3D(reactionSampler, uvwOld).x - k, 0.0);

	vec3 toCenter = uvw - HeatAreaCenter;
	if (dot(toCenter, toCenter) < HeatRadiusSq)
	{
		reaction = 1.0;
	}

	gl_FragColor = vec4(reaction);
}`